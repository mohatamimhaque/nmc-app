...
/**
 * PATCH /api/admin/volunteers/lunch
...
export async function PATCH(request: Request) {
  ...
  try {
    const { unique_id, is_lunch_collected } = await request.json()

    if (!unique_id || typeof unique_id !== 'string' || is_lunch_collected === undefined) {
      return NextResponse.json({ error: 'Missing or invalid unique_id or is_lunch_collected parameters.' }, { status: 400 })
    }
    ...
    const updatePayload = {
      is_lunch_collected: !!is_lunch_collected,
