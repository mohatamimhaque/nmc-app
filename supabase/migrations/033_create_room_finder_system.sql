...
select 'room_finder', 'Find Room', '/seat-location', true
where not exists (
...
select 'Find Room', '/seat-location', 12, true, false, false
where not exists (
  select 1 from public.nav_links where url = '/seat-location'
);
