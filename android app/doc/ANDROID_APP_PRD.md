...
graph TD
    ...
    F -->|Kit / Presents / Launch / Breakfast| H[6b. PATCH: Auto-Update Status]
...
3.  **Configure Mode in Settings**: The coordinator selects the active mode from the Settings page:
    ...
    *   **Breakfast Collect** (triggers status update PATCH to `/registrations/breakfast` on scan).
...
*   **Features**:
    ...
    - **Courtesy Credit**: A small, subtle footnote at the bottom of the screen reading `courtesy: mohatamim` in lowercase, utilizing low-contrast typography(in open screeen with logo nad in about page)
...
*   **Layout**: Grid of Material 3 glassmorphism cards showing registration summary statistics fetched from the `/api/admin/registrations/summary` API:
    ...
    *   **Breakfast Served**: `Served / Pending`
...
*   **Controls**:
    *   **Active Scan Mode Selector**: Radio buttons or list to select the active mode:
        ...
        4. **Breakfast Collect** (triggers breakfast status PATCH on scan)
    *   **Show Info Toggle**:
...
*   **Layout**: Scrollable, card-grouped details section showing the participant's full profile:
    1.  **Status Banners**:
        - In update modes (Kit/Presents/Launch/Breakfast): Shows a green alert card reading "Successfully Updated" (or amber if already processed).
    ...
    4.  **Live Status Badges**:
        ...
        - **Breakfast Served**: Teal (Served) / Gray (Pending)
    5.  **Inline Manual Toggles**: Fallback switches for all four status variables, allowing authorized editors to fix typos or perform manual override updates.
*   **Controls**: 
...
### Endpoint 3b: Update Breakfast Status
*   **Path**: `PATCH /api/admin/registrations/breakfast`
*   **Headers**:
    - `Content-Type: application/json`
    - `Authorization: Bearer <access_token>`
*   **Request Body**:
...
*   **Response (400 Bad Request)**:
    ```json
    {
      "error": "Missing or invalid serial or is_collect_breakfast parameters."
    }
    ```
...
*   **Tables**:
    ...
    2.  `offline_patches_queue`: Stores status change actions (PATCH payloads) triggered by scans while offline.
        -   Fields: `id` (autoincrement PK), `serial` (text), `action_type` (text: `"kit" | "present" | "launch" | "breakfast"`), `value` (boolean), `timestamp` (text/ISO).
